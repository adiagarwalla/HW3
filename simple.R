...
library(ggplot2)
#Plotting the violin plot for the simple analysis
...
LDA = read.csv("~/Desktop/COS/COS424/P3/HW3/lda_no_white.txt", header=TRUE, check.names=FALSE)
probLDA = rep(0, nrow(test.data))
for (i in 1:nrow(test.data))
{
  probLDA[i] = LDA[i]
  print(probLDA[i])
...
print(probLDA)
#Plotting the probability file generated by Aditya's lda code 
dat = data.frame(t(LDA), as.factor(test.data[,bool]))
colnames(dat) = c("prob", "value")
ggplot(dat, aes(value, prob)) + geom_violin() + geom_jitter(alpha=0.1) + 
  scale_y_log10() + labs(x="test value", y="prob")
